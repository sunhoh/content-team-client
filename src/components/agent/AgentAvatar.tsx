...
export function AgentAvatar({
  ...
  return (
    <div className='relative inline-flex shrink-0'>
      ...
      {status && (
        <span
          className={`border-card absolute -right-0.5 -bottom-0.5 h-2.5 w-2.5 rounded-full border-2 ${statusDotClass[status]}`}
        />
      )}
      {!status && (
        <span
          className={`border-card absolute -right-0.5 -bottom-0.5 h-2.5 w-2.5 rounded-full border-2 ${dotClass}`}
        />
      )}
    </div>
  );
